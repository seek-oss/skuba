...
describe("getHeadCommitId", () => {
  it("prefers a commit ID from a Buildkite environment", async () => {
    await expect(getHeadCommitId({ dir, env: { BUILDKITE_COMMIT: "b".repeat(40) } })).resolves.toBe(
      "b".repeat(40),
    );
  ...
  it("prefers a commit ID from a GitHub Actions environment", async () => {
    await expect(getHeadCommitId({ dir, env: { GITHUB_SHA: "c".repeat(40) } })).resolves.toBe(
      "c".repeat(40),
    );
  ...
  it("falls back to a commit ID from the Git log", async () => {
    ...
    await expect(getHeadCommitId({ dir, env: {} })).resolves.toBe("a".repeat(40));
  ...
  it("throws on an empty Git log", async () => {
    ...
    await expect(getHeadCommitId({ dir, env: {} })).rejects.toThrowErrorMatchingInlineSnapshot(
...
describe("getHeadCommitMessage", () => {
  ...
  it("falls back to a commit ID from the Git log", async () => {
    jest.mocked(git.log).mockResolvedValue([{ commit: { message: "Do work" } } as any]);

    await expect(getHeadCommitMessage({ dir, env: {} })).resolves.toBe("Do work");
  ...
  it("throws on an empty Git log", async () => {
    ...
    await expect(getHeadCommitMessage({ dir, env: {} })).rejects.toThrowErrorMatchingInlineSnapshot(
      `"Git log does not contain any commits"`,
    );
