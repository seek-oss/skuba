...
beforeEach(() => jest.mocked(createRestClient).mockResolvedValue(mockClient as never));

afterEach(jest.resetAllMocks);

describe("getPullRequestNumber", () => {
  it("prefers a Buildkite environment variable", async () => {
    await expect(getPullRequestNumber({ env: { BUILDKITE_PULL_REQUEST: "123" } })).resolves.toBe(
      123,
    );
  ...
  it("throws on an empty response from the GitHub API", async () => {
    ...
    await expect(getPullRequestNumber({ env: {} })).rejects.toThrowErrorMatchingInlineSnapshot(
