...
describe("noop", () => {
  it("passes through value", () => expect(parsers.noop("abc", "VERSION")).toBe("abc"));
});

describe("oneOf", () => {
  const parse = parsers.oneOf(["local", "prod"]);

  it("passes through value in list", () => expect(parse("prod", "ENVIRONMENT")).toBe("prod"));

  it("throws on value not in list", () =>
    expect(() => parse("dev", "ENVIRONMENT")).toThrowErrorMatchingInlineSnapshot(
...
describe("boolean", () => {
  ...
  it.each(happyCases)("accepts %p", (input) => expect(parsers.boolean(input)).toBe(true));

  const sadCases = ["", "false", "no", "off", "0", "False", "FALSE", "n"];

  it.each(sadCases)("rejects %p", (input) => expect(parsers.boolean(input)).toBe(false));
});
