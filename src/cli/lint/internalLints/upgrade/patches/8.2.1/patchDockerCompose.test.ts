...
describe("patchDockerComposeFile", () => {
  ...
  it("should skip if no Dockerfile is found", async () => {
    ...
    await expect(tryPatchDockerComposeFiles({ mode: "format" } as PatchConfig)).resolves.toEqual({
      result: "skip",
  ...
  it("should patch docker-compose files with version field", async () => {
    ...
    jest.mocked(fs.readFile).mockResolvedValueOnce(mockPatchableDockerComposeContents as never);
    await expect(tryPatchDockerComposeFiles({ mode: "format" } as PatchConfig)).resolves.toEqual({
      result: "apply",
    });
  ...
  it("should skip if no docker-compose files contain a version field", async () => {
    jest.mocked(fg).mockResolvedValueOnce([mockDockerComposeFile]);
    jest.mocked(fs.readFile).mockResolvedValueOnce(mockDockerComposeContents as never);
    await expect(tryPatchDockerComposeFiles({ mode: "format" } as PatchConfig)).resolves.toEqual({
      result: "skip",
