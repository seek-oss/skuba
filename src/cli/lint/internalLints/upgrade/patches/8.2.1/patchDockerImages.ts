...
const isInvalidPlatformFlagUsage = (contents: string) => {
  ...
  const uniquePlatforms = [...new Set(matches.map(([, , platform]) => platform as string))];
...
const patchDockerImages: PatchFunction = async ({ mode }): Promise<PatchReturnType> => {
  ...
  const dockerFilePatches = dockerFilesToPatch.map(async ({ file, contents }) => {
    let patchedContents = contents.replace(DOCKER_IMAGE_REGEX, `$1$2${PUBLIC_ECR}$3$4`);

    if (isInvalidPlatformFlagUsage(contents)) {
      patchedContents = patchedContents.replace(DOCKER_IMAGE_PLATFORM_REGEX, "$1");
    }

    await fs.writeFile(file, patchedContents);
  });

  const dockerComposeFilePatches = dockerComposeFilesToPatch.map(async ({ file, contents }) => {
    const patchedContents = contents.replace(DOCKER_COMPOSE_IMAGE_REGEX, `$1${PUBLIC_ECR}$2$3`);
    await fs.writeFile(file, patchedContents);
  });
