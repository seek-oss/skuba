...
describe("patchJestSnapshots", () => {
  ...
  it("should skip if test files do not contain the old URL", async () => {
    ...
    jest.mocked(fs.readFile).mockResolvedValueOnce("No snapshot URL here" as never);
    await expect(
      tryPatchJestSnapshots({
        mode: "format",
      } as PatchConfig),
  ...
  it("should patch test files", async () => {
    jest.mocked(fg).mockResolvedValueOnce(["test1.test.ts", "test2.test.ts", "test3.test.ts.snap"]);
    jest
      .mocked(fs.readFile)
      .mockResolvedValueOnce("Some content with https://goo.gl/fbAQLP" as never)
      .mockResolvedValueOnce("No snapshot URL here" as never)
      .mockResolvedValueOnce("Some other content with https://goo.gl/fbAQLP" as never);
