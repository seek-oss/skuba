...
export const patchJestSnapshots = async (mode: "lint" | "format"): Promise<PatchReturnType> => {
  ...
  await Promise.all(
    testFilesToPatch.map(async ({ file, contents }) => {
      const updatedContents = contents.replaceAll(JEST_SNAPSHOT_OLD_URL, JEST_SNAPSHOT_NEW_URL);
