...
const removeVitestImports: PatchFunction = async ({ mode }): Promise<PatchReturnType> => {
    ...
    const files = await Promise.all(allTsFiles.map(async (file) => {
    ...
    const changedFiles = await Promise.all(files.map(async ({ file, contents }) => {
    ...
    await Promise.all(actualChangedFiles.map(async ({ file, updated }) => {
...
export const tryRemoveVitestImports: PatchFunction = async (config): Promise<PatchReturnType> => {
  try {
