...
describe("patchDockerfileCIVariable", () => {
  ...
  it("should skip if dockerfiles do not contain pnpm install --prod commands", async () => {
    ...
    jest.mocked(fs.readFile).mockResolvedValueOnce("FROM node:18\nRUN npm install" as never);
    await expect(
      tryPatchDockerfileCIVariable({
        mode: "format",
      } as PatchConfig),
  ...
  it("should patch multiple dockerfiles containing pnpm install --prod commands", async () => {
    jest.mocked(fg).mockResolvedValueOnce(["Dockerfile", "Dockerfile.dev", "Dockerfile.prod"]);
...
CMD ["npm", "start"]`;
    ...
    expect(fs.writeFile).toHaveBeenCalledWith("Dockerfile", expectedContent, "utf8");
  });
