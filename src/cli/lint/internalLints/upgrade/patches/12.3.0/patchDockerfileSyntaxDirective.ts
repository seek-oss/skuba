...
export const patchDockerfileSyntaxDirective = async (
  ...
  const dockerfilesToPatch = dockerfiles.filter(({ contents }) => dockerSyntaxRegex.test(contents));
...
export const tryPatchDockerfileSyntaxDirective: PatchFunction = async ({ mode }) => {
  try {
