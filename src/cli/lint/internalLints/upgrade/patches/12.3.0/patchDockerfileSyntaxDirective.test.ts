...
describe("patchDockerfileSyntaxDirective", () => {
  ...
  it("should skip if dockerfiles do not contain the Dockerfile syntax directive", async () => {
    ...
    jest.mocked(fs.readFile).mockResolvedValueOnce("No Dockerfile syntax directive here" as never);
    await expect(
      tryPatchDockerfileSyntaxDirective({
        mode: "format",
      } as PatchConfig),
  ...
  it("should return apply and not modify files if mode is lint", async () => {
    jest.mocked(fg).mockResolvedValueOnce(["Dockerfile"]);
    jest.mocked(fs.readFile).mockResolvedValueOnce("# syntax=docker/dockerfile:1.18\n" as never);
  ...
  it("should patch dockerfiles if mode is format", async () => {
    jest
      .mocked(fg)
      .mockResolvedValueOnce(["Dockerfile", "Dockerfile.dev-deps", "Dockerfile.build"]);
    jest
      .mocked(fs.readFile)
    ...
    expect(fs.writeFile).toHaveBeenCalledWith("Dockerfile", "FROM node:22", "utf8");
    expect(fs.writeFile).toHaveBeenCalledWith("Dockerfile.dev-deps", "FROM python:3.9", "utf8");
