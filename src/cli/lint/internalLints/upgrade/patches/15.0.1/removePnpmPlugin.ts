...
export const removePnpmPlugin: PatchFunction = async ({ mode }): Promise<PatchReturnType> => {
  ...
  if (node) {
    const mappingItems = node.findAll({ rule: { kind: "block_mapping_pair" } }).slice(1); // skip the first mapping pair which is 'configDependencies'
    ...
    if (pnpmPluginSkubaNode) {
      ...
      await fs.promises.writeFile(path.join(dir, "pnpm-workspace.yaml"), updated, "utf8");
