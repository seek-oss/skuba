...
const patchPnpmPackageManager: PatchFunction = async ({
  ...
  if (!(manifest.packageJson as { packageManager?: string }).packageManager?.includes("pnpm")) {
    return {
      result: "skip",
  ...
  const dockerFilesToPatch = dockerfiles.filter(
    ({ contents }) =>
      contents.includes(DOCKERFILE_COREPACK_COMMAND) && !contents.includes("target=package.json"),
  );
  ...
  if (dockerFilesToPatch.length) {
    await Promise.all(
      dockerFilesToPatch.map(async ({ file, contents }) => {
        const patchedContent = contents.replace(DOCKERFILE_COREPACK_COMMAND, PACKAGE_JSON_MOUNT);
  ...
  if (pipelinesToPatch.length) {
    await Promise.all(
      pipelinesToPatch.map(async ({ file, contents }) => {
        ...
        const patchedEcrContent = patchedContent.replace(ECR_REGEX, (match, version) => {
