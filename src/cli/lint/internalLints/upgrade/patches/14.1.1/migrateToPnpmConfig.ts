...
export const migrateToPnpmConfig: PatchFunction = async ({ mode }): Promise<PatchReturnType> => {
  ...
  const endSection = pnpmWorkSpaceFile.slice(endOfSectionIndex + endOfSectionIndexText.length);
  ...
  if (typeof packageJson?.packageJson.packageManager === "string") {
    ...
    if (typeof version === "string" && lt(new SemVer(version), new SemVer("10.26.1"))) {
