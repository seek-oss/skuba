...
import { REFRESHABLE_CONFIG_FILES, refreshConfigFiles } from "./refreshConfigFiles.js";
...
const givenMockPackageManager = (command: "pnpm" | "yarn") => {
  jest
    ...
    .mockResolvedValue(
      jest.requireActual("../../../utils/packageManager").configForPackageManager(command),
    );
};
...
describe("refreshConfigFiles", () => {
  ...
  const createDestinationFileReader = jest.mocked(project.createDestinationFileReader);
