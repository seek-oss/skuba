...
import { AUTOFIX_IGNORE_FILES_BASE, AUTOFIX_IGNORE_FILES_NPMRC, autofix } from "./autofix.js";
...
beforeEach(() => {
  ...
  jest.mocked(createDestinationFileReader).mockReturnValue(jest.fn().mockResolvedValue(null));
});
...
describe("autofix", () => {
  ...
  describe("GitHub Actions", () => {
    ...
    it("bails on a renovate branch when there is no open pull request", async () => {
      ...
      jest
        ...
        .mockRejectedValue(
          new Error(`Commit cdd1520 is not associated with an open GitHub pull request`),
        );
    ...
    it("bails on an autofix head commit", async () => {
      ...
      jest.mocked(Git.getHeadCommitMessage).mockResolvedValue("Run `skuba format`\n");
    ...
    it("handles fixable issues from ESLint only", async () => {
      ...
      await expect(autofix({ ...params, eslint: true, oxfmt: false })).resolves.toBeUndefined();
  ...
  describe("Other CI", () => {
    ...
    it("handles internal changes only", async () => {
      ...
      await expect(autofix({ ...params, eslint: false, oxfmt: false })).resolves.toBeUndefined();
