...
const normalizeVersionRange = (currentVersion: string, newVersion: string): string => {
...
export const upgradeInfraPackages = async (
    ...
    .map((file) => {
      const updated = packages.reduce((contents, packageName) => {
        ...
        return contents.replaceAll(regex, (match, currentVersion: string) => {
          ...
          const newVersion = normalizeVersionRange(currentVersion, packageName.version);
  ...
  await Promise.all(
    [...patchedPackageJsons, ...patchedPnpmWorkspaces].map(async ({ file, updated }) => {
  ...
  await exec(packageManager.command, "install", "--frozen-lockfile=false", "--prefer-offline");
