...
import { getEntryPointFromManifest, getManifestProperties } from "../../utils/manifest.js";

export const copyAssets = async (destinationDir: string, logger: Logger = log) => {
  ...
  const resolvedDestinationDir = path.resolve(path.dirname(manifest.path), destinationDir);
  ...
  const matchedFiles = Array.from(new Set(Object.values(filesByPattern).flat()));

  await Promise.all(
    matchedFiles.map(async (filename) => {
      logger.subtle(`Copying ${filename}`);

      await fs.promises.mkdir(path.dirname(path.join(resolvedDestinationDir, filename)), {
        recursive: true,
      });
      await copyFile(
