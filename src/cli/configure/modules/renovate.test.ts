import { defaultOpts, defaultPackageOpts, executeModule } from "../testing/module.js";
...
describe("renovateModule", () => {
  it("works from scratch", async () => {
    ...
    const outputFiles = await executeModule(renovateModule, inputFiles, defaultOpts);

    expect(outputFiles[".github/renovate.json5"]).toContain("seek-oss/rynovate");
  ...
  it("makes packages publishable", async () => {
    ...
    const outputFiles = await executeModule(renovateModule, inputFiles, defaultPackageOpts);
...
`);
  ...
  it("migrates extended config", async () => {
    ...
    expect(outputFiles[".github/renovate.json5"]).toBe("{ extends: ['seek'] }\n");
