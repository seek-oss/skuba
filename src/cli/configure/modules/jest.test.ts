...
describe("jestModule", () => {
  it("works from scratch", async () => {
    ...
    const outputFiles = await executeModule(jestModule, inputFiles, defaultOpts);
  ...
  it("migrates Jest options", async () => {
    ...
    expect(outputFiles["jest.config.ts"]).toContain("globalSetup: './globalSetup.js',");
...
`,
  ...
  ])("strips outdated `isolatedModules` config snippets $description", async ({ config }) => {
    const inputFiles = {
