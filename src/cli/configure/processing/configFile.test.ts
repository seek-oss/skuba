import { generateIgnoreFileSimpleVariants, mergeWithConfigFile } from "./configFile.js";
...
describe("generateIgnoreFileSimpleVariants", () => {
  it.each([
    ...
    ["file extension", ["*.tgz"], ["*.tgz", ".tgz", ".tgz/", "/.tgz", "/.tgz/"]],
    ["potential empty string", ["/"], ["/"]],
    ["empty string", [""], []],
  ])("handles %s", (_, pattern, expected) =>
    expect(generateIgnoreFileSimpleVariants(pattern)).toEqual(new Set(expected)),
  );
});
