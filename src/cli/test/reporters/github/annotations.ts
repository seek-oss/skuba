...
export const createAnnotations = (testResults: TestResult[]): GitHub.Annotation[] => {
...
export const generateAnnotationEntries = (testResults: TestResult[]): AnnotationEntry[] => {
  type ResultsByDisplayName = Record<string | symbol, TestResult[]>;

  // Group test results by display name.
  const resultsByDisplayName = testResults.reduce<ResultsByDisplayName>((acc, result) => {
