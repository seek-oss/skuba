import { hasDebugFlag, hasSerialFlag, parseProcessArgs, parseRunArgs } from "./args.js";
...
describe("hasDebugFlag", () => {
  ...
  `("$description => $expected", ({ args, expected }) => expect(hasDebugFlag(args)).toBe(expected));
});
...
describe("parseProcessArgs", () => {
  ...
  it("parses a macOS command without args", () => {
    const argv = ["/usr/local/bin/node", "/Users/user/repo/node_modules/.bin/skuba", "start"];
