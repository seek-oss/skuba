...
import { buildPatternToFilepathMap, crawlDirectory, createInclusionFilter } from "./dir.js";
...
describe("createInclusionFilter", () => {
  it("gracefully handles non-existent .gitignore", () => {
    const promise = createInclusionFilter(["wjifojwf9032jf930jf902jf902jf90j09"]);
  ...
  describe("with sensible template defaults", () => {
    ...
    beforeAll(async () => {
      include = await createInclusionFilter([path.join(BASE_TEMPLATE_DIR, "_.gitignore")]);
    });
