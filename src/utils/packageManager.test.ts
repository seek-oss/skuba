...
describe("detectPackageManager", () => {
  it("detects pnpm", async () => {
    findUp.mockImplementation((file: string) =>
      Promise.resolve(file === "pnpm-lock.yaml" ? "/root/pnpm-lock.yaml" : undefined),
    );
  ...
  it("preferences yarn on confusing project setups with sibling lockfiles", async () => {
    findUp.mockImplementation((file: string) =>
      Promise.resolve(file === "pnpm-lock.yaml" ? "/root/pnpm-lock.yaml" : "/root/yarn.lock"),
    );
  ...
  it("preferences the closest lockfile if at different levels", async () => {
    findUp.mockImplementation((file: string) =>
      Promise.resolve(file === "pnpm-lock.yaml" ? "/root/a/b/c/pnpm-lock.yaml" : "/root/yarn.lock"),
    );
