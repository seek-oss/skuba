...
export const configForPackageManager = (packageManager: PackageManager): PackageManagerConfig => ({
  ...PACKAGE_MANAGERS[packageManager],
  command: packageManager,
});

export const detectPackageManager = async (cwd?: string): Promise<PackageManagerConfig> => {
  ...
  try {
    ...
    log.warn(
      `Failed to detect package manager; defaulting to ${log.bold(DEFAULT_PACKAGE_MANAGER)}.`,
    );
    log.subtle(
      (() => {
        switch (true) {
