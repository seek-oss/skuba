...
const getNpmPackage = async (packageName: string) => {
  try {
    ...
    if (!parsedResponse.success) {
      throw new Error(`Failed to parse package response from npm for package ${packageName}`);
    }
...
export const getNpmVersions = async (packageName: string): Promise<NpmVersions | null> => {
  const response = await getNpmPackage(packageName);
  return response?.versions ?? null;
};

export const getLatestNpmVersion = async (packageName: string): Promise<string | null> => {
  const response = await getNpmPackage(packageName);
...
export const getSkubaVersionInfo = async (): Promise<SkubaVersionInfo> => {
  const [local, latest] = await Promise.all([getSkubaVersion(), latestSkubaVersion()]);
