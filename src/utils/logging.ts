...
export const createLogger = ({
  ...
  const logWithoutSuffixes = (...message: unknown[]) => console.log(...prefixes, ...message);

  const log = (...message: unknown[]) => logWithoutSuffixes(...message, ...suffixes);
  ...
  return {
    ...
    timing: (start: bigint, end: bigint) => `${Number((end - start) / BigInt(10_000_000)) / 100}s`,
...
export const childLogger = (logger: Logger, settings: Partial<Logger["settings"]>) =>
  createLogger({
