...
var log = function (level, msg, extra) {
    ...
    var toLog = __assign({ msg: msg, level: level, app: app, time: new Date().toISOString() }, extra);
...
var main = function () {
    ...
    var sdk = new sdk_node_1.NodeSDK({
        ...
        instrumentations: [new instrumentation_http_1.HttpInstrumentation(), new instrumentation_aws_sdk_1.AwsInstrumentation()]
    });
    sdk
        .start()
        .then(function () { return log('info', 'OpenTelemetry initialised'); })["catch"](function (err) {
        return log('error', 'OpenTelemetry not initialised', { err: err });
    });
    process.on('SIGTERM', function () {
        sdk
            .shutdown()
            .then(function () { return log('info', 'OpenTelemetry successfully terminated'); })["catch"](function (err) {
            return log('error', 'OpenTelemetry failed to terminate', { err: err });
        })["finally"](function () { return process.exit(0); }); // eslint-disable-line no-process-exit
    });
};
