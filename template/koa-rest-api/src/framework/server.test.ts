...
describe("createApp", () => {
  ...
  it("handles root route", async () => {
    ...
    await agent.get("/").expect(200, "").expect("server", /.+/).expect("x-api-version", /.+/);
  ...
  it("handles returned client error", async () => {
    ...
    await agent.get("/").expect(400, message).expect("server", /.+/).expect("x-api-version", /.+/);
  ...
  it("handles server error thrown from context", async () => {
    ...
    await agent.get("/").expect(500, "").expect("server", /.+/).expect("x-api-version", /.+/);
