...
it.each(["dev", "prod"])("returns expected CloudFormation stack for %s", async (deployment) => {
  ...
  jest
    ...
    .mockImplementation((scope, id) => new aws_secretsmanager.Secret(scope, id));
  ...
  const json = JSON.stringify(template.toJSON())
    .replace(/"S3Key":"([0-9a-f]+)\.zip"/g, (_, hash) => `"S3Key":"${"x".repeat(hash.length)}.zip"`)
    .replace(
    ...
    .replace(/"DD_TAGS":"git.commit.sha:([0-9a-f]+),git.repository_url:([^\"]+)",/g, "")
