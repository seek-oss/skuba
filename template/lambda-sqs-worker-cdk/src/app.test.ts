...
describe("handler", () => {
  ...
  const distribution = jest.spyOn(metricsClient, "distribution").mockReturnValue();
  ...
  it("handles multiple records", async () => {
    const event = createSqsEvent([JSON.stringify(jobPublished), JSON.stringify(jobPublished)]);
  ...
  it("handles partial batch failure", async () => {
    const event = createSqsEvent([JSON.stringify("}"), JSON.stringify(jobPublished)]);
