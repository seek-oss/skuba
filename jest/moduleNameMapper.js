...
module.exports.createModuleNameMapper = (getConfig) => {
  ...
  const paths = Object.fromEntries(
    Object.entries(maybeTsConfig?.options.paths ?? {}).flatMap(([key, values]) => [
      ...
      [key.replace(/\/$/, ""), values.map((value) => value.replace(/\/$/, ""))],
  ...
  return Object.fromEntries(
    Object.entries(moduleNameMapper).map(([key, values]) => [
      key,
      Array.isArray(values) ? values.map((value) => path.normalize(value)) : path.normalize(values),
    ]),
  );
};
